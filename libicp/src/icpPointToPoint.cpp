...
#pragma omp parallel for private(i) default(none) shared(T,active,nact,p_m,p_t,r00,r01,r02,r10,r11,r12,r20,r21,r22,t0,t1,t2) reduction(+:mum0,mum1,mum2, mut0,mut1,mut2) // schedule (dynamic,2)
    for (i=0; i<nact; i++) {
      ...
      p_m.val[i][0] = m_kd_tree->the_data[result[0].idx][0]; mum0 += p_m.val[i][0];
      p_m.val[i][1] = m_kd_tree->the_data[result[0].idx][1]; mum1 += p_m.val[i][1];
      p_m.val[i][2] = m_kd_tree->the_data[result[0].idx][2]; mum2 += p_m.val[i][2];
...
double IcpPointToPoint::getResidual( double *T,const int32_t T_num,const Matrix &R,const Matrix &t,const std::vector<int> &active )
{
